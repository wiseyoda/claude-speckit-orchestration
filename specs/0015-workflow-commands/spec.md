...
**Independent Test**: Run `/speckit.merge` after completing a phase's implementation. Verify: branch is pushed, PR created (or merged if auto-merge enabled), state archived, ROADMAP updated, and backlog summary shown.

**Acceptance Scenarios**:

1. **Given** a completed phase on feature branch, **When** I run `/speckit.merge`, **Then** the branch is pushed, PR is created and merged, feature branch is deleted, and status is shown.
2. **Given** a completed phase, **When** I run `/speckit.merge --pr-only`, **Then** the branch is pushed, PR is created but NOT merged, and I'm given the PR URL to review.
3. **Given** state shows `orchestration.step.current=verify` with all tasks complete, **When** I run `/speckit.merge`, **Then** state is archived and ROADMAP shows phase as complete.
4. **Given** the phase has incomplete tasks, **When** I run `/speckit.merge`, **Then** I receive a warning with list of incomplete tasks and option to continue or abort.
...
**Independent Test**: Run `/speckit.backlog` with existing backlog items. Verify: items are analyzed, matched to phases by theme, and ROADMAP updated with assignments. Unassignable items create new phases.

**Acceptance Scenarios**:

1. **Given** ROADMAP has a Backlog section with items, **When** I run `/speckit.backlog`, **Then** I see each item analyzed with suggested phase assignment.
...
**Independent Test**: Run `speckit roadmap backlog add "New feature idea"`. Verify: item appears in ROADMAP Backlog section with timestamp.

**Acceptance Scenarios**:

1. **Given** ROADMAP has a Backlog section, **When** I run `speckit roadmap backlog add "My idea"`, **Then** "My idea" is appended to the Backlog section.
2. **Given** ROADMAP has no Backlog section, **When** I run `speckit roadmap backlog add "My idea"`, **Then** a Backlog section is created with the item.
...
- What happens when the feature branch has unpushed commits?
  - `/speckit.merge` should detect and push them before PR creation
...
- What happens when backlog is empty?
  - `/speckit.backlog` shows "No items to triage" and exits cleanly
...
- **FR-001**: `/speckit.merge` MUST push current branch to remote before any merge operations
- **FR-002**: `/speckit.merge` MUST verify all tasks are complete before proceeding (with override option)
- **FR-003**: `/speckit.merge` MUST update `orchestration-state.json` via `speckit state archive` command
- **FR-004**: `/speckit.merge` MUST update ROADMAP.md phase status to "Complete"
- **FR-005**: `/speckit.merge` MUST display backlog summary at completion
- **FR-006**: `/speckit.backlog` MUST analyze each backlog item against existing phase scopes
- **FR-007**: `/speckit.backlog` MUST propose new phases for unassignable items
- **FR-008**: `/speckit.backlog` MUST update ROADMAP.md with assignments
- **FR-009**: `speckit roadmap backlog add` MUST append item to ROADMAP Backlog section
- **FR-010**: `speckit roadmap backlog add` MUST create Backlog section if missing
...
- **SC-001**: Developer can complete a phase with single `/speckit.merge` command (no manual git/state steps needed)
...
**Question**: Should `/speckit.merge` automatically merge the PR to main?
...
**Question**: Should `/speckit.merge` delete the feature branch after successful merge?
...
- State changes MUST go through `speckit state` CLI, not direct file edits
