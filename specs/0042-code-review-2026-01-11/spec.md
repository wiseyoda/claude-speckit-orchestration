...
As a developer using SpecKit on older bash versions or different shells, I can run all CLI scripts without encountering bash 4.0+ compatibility errors.
...
**Independent Test**: Run `speckit doctor` and `speckit reconcile` on a POSIX shell (dash) or bash 3.x - should work without errors.

**Acceptance Scenarios**:

1. **Given** bash 3.x environment, **When** running `speckit doctor`, **Then** script executes without "declare -a" or "declare -A" errors
2. **Given** bash 3.x environment, **When** running `speckit reconcile`, **Then** script executes without associative array errors
3. **Given** any SpecKit script, **When** analyzed by shellcheck, **Then** no POSIX compatibility warnings
...
**Independent Test**: Run `speckit help`, `speckit feature list` - all phase examples and outputs show 4-digit format.

**Acceptance Scenarios**:

1. **Given** `bin/speckit help`, **When** displaying phase examples, **Then** all examples use 4-digit format (e.g., 0010, not 001)
2. **Given** `speckit feature list`, **When** listing features, **Then** phase numbers are displayed in 4-digit format

---

### User Story 3 - Complete CLI Dispatcher (Priority: P2)

As a developer, I can access `gate` and `lessons` commands through the main `speckit` CLI dispatcher, matching the documented command set.

**Why this priority**: Missing dispatcher commands break the expected CLI interface.

**Independent Test**: Run `speckit gate --help` and `speckit lessons --help` - both should work.

**Acceptance Scenarios**:

1. **Given** main CLI dispatcher, **When** running `speckit gate`, **Then** command routes to gate script
2. **Given** main CLI dispatcher, **When** running `speckit lessons`, **Then** command routes to lessons script
...
3. **Given** README.md, **When** looking for review command, **Then** `/speckit.review` is listed
...
- **FR-BP001**: `speckit-doctor.sh` MUST NOT use `declare -a` (bash 4.0+ feature)
- **FR-BP002**: `speckit-reconcile.sh` MUST NOT use `declare -a` or `declare -A`
- **FR-BP003**: All scripts MUST use `log_error`/`log_warn` from common.sh consistently
- **FR-BP004**: `speckit-feature.sh` MUST display 4-digit phase numbers
- **FR-BP005**: `bin/speckit` help MUST show 4-digit phase examples

#### Refactoring (RF003-RF004)
- **FR-RF003**: `speckit-doctor.sh` SHOULD have cleaner check abstraction
- **FR-RF004**: `lib/common.sh` SHOULD have unused functions removed

#### Hardening (HD001-HD003)
- **FR-HD001**: `speckit-gate.sh` MUST handle test runner errors gracefully
- **FR-HD002**: All scripts with temp files MUST have EXIT trap cleanup
- **FR-HD003**: `speckit-import.sh` MUST validate ADR format before import

#### Missing Features (MF001-MF003)
- **FR-MF001**: `bin/speckit` MUST dispatch `gate` and `lessons` subcommands
- **FR-MF002**: `speckit-gate.sh` SHOULD detect cargo test, go test runners
- **FR-MF003**: `speckit-context.sh` SHOULD show memory document status

#### Orphaned Code (OC001-OC002)
- **FR-OC001**: `speckit-context.sh` MUST remove unused `include_tasks` variable
...
- **FR-OD003**: README.md MUST list `/speckit.review` command
...
- **SC-004**: `speckit doctor` runs successfully on POSIX-compliant shell
