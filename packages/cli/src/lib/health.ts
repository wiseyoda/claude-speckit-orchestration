...
async function collectIssues(projectPath?: string): Promise<HealthIssue[]> {
  ...
  if (!root) {
    issues.push({
      ...
      fix: 'Navigate to a SpecFlow project or run "specflow project init"',
      autoFixable: false,
    });
    return issues;
  }
  ...
  if (!pathExists(statePath)) {
    issues.push({
      ...
      fix: 'Run "specflow state init" to initialize project state',
      autoFixable: true,
    });
  ...
  try {
    ...
    issues.push({
      ...
      fix: 'Run "specflow state init --force" to reset state',
      autoFixable: true,
    });
    return issues;
  }
  ...
  if (!pathExists(memoryDir)) {
    issues.push({
      ...
      fix: 'Run "specflow init" to set up memory documents',
      autoFixable: false,
    });
  }
      ...
      if (!roadmapPhase) {
        ...
        if (stateStatus === 'in_progress' && roadmapStatus === 'complete') {
          issues.push({
            ...
            fix: 'Run "specflow state archive" to archive completed phase',
            autoFixable: true,
          });
        }
      }
      ...
      if (currentBranch && currentBranch !== expectedBranch) {
        ...
        if (currentBranch === 'main' || currentBranch === 'master') {
          issues.push({
            ...
            fix: 'If phase was merged, run "specflow state archive" to start next phase',
            autoFixable: false,
          });
        } else {
          issues.push({
